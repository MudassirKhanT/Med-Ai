...
export async function POST(req: Request) {
  try {
    ...
    const text = notes.toLowerCase();
    const matchedDoctors = AIDoctorAgents.filter((doctor) => doctor.keywords.some((keyword) => text.includes(keyword)));

    // fallback to general if no match
    const doctorsToSuggest = matchedDoctors.length > 0 ? matchedDoctors : [AIDoctorAgents.find((doc) => doc.specialist === "General Physician")!];

    return NextResponse.json({ suggestedDoctors: doctorsToSuggest });
  } catch (err) {
